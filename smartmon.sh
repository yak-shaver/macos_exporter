...
parse_smartctl_info() {
  ...
  while read line; do
    info_type="$(echo "${line}" | cut -f1 -d: | tr ' ' '_')"
		info_value="$(echo "${line}" | cut -f2- -d: | xargs)"
    case "${info_type}" in
...
bin=/usr/local/bin/smartctl
smartctl_version="$($bin -V | head -n1 | awk '$1 == "smartctl" {print $2}')"
...
device_list="$($bin --scan-open | awk '/^IOService/{print $1 "|" $3}')"
...
for device in ${device_list}; do
  ...
  $bin -i -H -d "${type}" "${disk}" | parse_smartctl_info "${disk}" "${type}"
done | format_output
